...
function MyLooks(props) {
  ...
  if (state.newLook) {
    ...
    state.userLooks.map((element) => {
      return (
        <div className='look'>
            {element.photo ? <div className='wrapper'><div className='photo-container'>{element.photo}</div></div> : <div className='wrapper'><div className='multi-photo-container'><img src={element.head} alt='legs'  /> <img src={element.body} alt='body'  /><img src={element.legs} alt='legs'  /><img src={element.feet} alt='feet'  /></div></div>}
          <h2>{element.name}</h2>
          <div className='tags'>
            {element.tags.map((element2) => {
              return <div className='tag'>{element2}</div>;
